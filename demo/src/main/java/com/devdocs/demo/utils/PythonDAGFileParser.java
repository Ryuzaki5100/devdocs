...
public class PythonDAGFileParser {
    ...
    private static final String PYTHON_ANALYZE_SCRIPT = """
            ...
            class DAGParser(ast.NodeVisitor):
                def __init__(self):
                    self.dependencies = set()  # Unique chains
            
                def visit_BinOp(self, node):
                    if isinstance(node.op, ast.RShift):  # Detect ">>" operator
                        chain = self._build_dependency_chain(node)
                        if chain:
                            self.dependencies.add(chain)
                ...
                def _build_dependency_chain(self, node):
                    if isinstance(node, ast.BinOp) and isinstance(node.op, ast.RShift):
                        ...
                        if left and right:
                            chain = f"{left} >> {right}"
                            self.dependencies.add(chain)  # Store intermediate dependencies
                            return chain
                ...
                def report_json(self):
                    chains = sorted(self.dependencies, key=lambda x: x.count(">>"))  # Sort by length
            
                    if len(chains) == 1:
                        return {"DAG": [chains[0]]}  # Single line
            
                    longest_chain = chains[-1]  # Full chain
                    second_chain = next((c for c in reversed(chains[:-1]) if not c in longest_chain), None) \s
            
                    if second_chain:
                        return {"DAG": [second_chain, longest_chain]}  # Two meaningful steps
                    else:
                        return {"DAG": [longest_chain]}  # Still just one
